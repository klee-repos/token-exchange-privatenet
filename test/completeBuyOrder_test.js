...
contract ('completeBuyOrder_test', function(accounts) {

	var sellPrice_1 = 30000;
	var sellQuantity_1 = 3;
	...
	var sellPrice_3 = 10000;
	var sellQuantity_3 = 3;
	var sellTotal_3 = sellPrice_3 * sellQuantity_3;

	var buyPrice_1 = 20000;
	var buyQuantity_1 = 4;
	...
	it('Create and complete buy order on Exchange', function() {
		...
		return exchange.deployed().then(function(instance) {
			...
			assert.equal(txResults.logs[0].event, "buyOrderFulfilled", "buyOrderFulfilled event should have fired here" );
			return exchangeInstance.getTokenBalance.call("FIXED", {from: accounts[1]});
		}).then(function(balance) {
			assert.equal(balance.toNumber(), buyQuantity_1, "account 1 token balance is incorrect");
			return exchangeInstance.getSellOrderBook.call("FIXED", {from: accounts[1]});
		}).then(function(orderbook) {
			assert.equal(orderbook[0].length, 2, "sell order book length is incorrect");
			assert.equal(orderbook[1][0].toNumber(), 2, "sell order book values are incorrect");
